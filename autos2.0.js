...
function cruzarLeads() {
  ...
  if (!hojaCruzar || !hojaAcumulado) {
    Logger.log('Error: No se encontraron las hojas con los nombres especificados.');
    SpreadsheetApp.getUi().alert('Error: Asegúrate de que los nombres de las hojas sean correctos.');
    return;
  }
  ...
  for (let i = 1; i < datosAcumulado.length; i++) {
    ...
    const cc = fila[COL_CC_ACUMULADO]?.toString().trim() || '';
    const correo = fila[COL_CORREO_ACUMULADO]?.toString().trim() || '';
  ...
  for (let i = 1; i < datosCruzar.length; i++) {
    ...
    const utmSource = fila[COL_UTM_SOURCE]?.toString().toLowerCase().trim() || '';
    let valorEncontrado = '';

    if (utmSource === 'ig' || utmSource === 'fb') {
      const cc = fila[COL_CC_CRUZAR]?.toString().trim() || '';
      const correo = fila[COL_CORREO_CRUZAR]?.toString().trim() || '';
