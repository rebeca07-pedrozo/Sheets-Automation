function EmisionesAutosCruzados(nombreHoja) {
  ...
  const esCorreoValido = c => typeof c === 'string' && c.includes('@');
  ...
  const hojaLF = ss.getSheetByName("LEADS_FALTANTES");
  if (!hojaLF) {
    Logger.log("Advertencia: No se encontró la hoja 'LEADS_FALTANTES'. Se omite esta base.");
  }

  const mapLF_CC = new Map(), mapLF_Placa = new Map(), mapLF_Correo = new Map();

  let leadsFaltantesDatos = [];
  if (hojaLF) {
    const ultimaFilaLF = hojaLF.getLastRow();
    const ultimaColumnaLF = hojaLF.getLastColumn();
    leadsFaltantesDatos = (ultimaFilaLF > 1 && ultimaColumnaLF > 0)
      ? hojaLF.getRange(2, 1, ultimaFilaLF - 1, ultimaColumnaLF).getValues()
      : [];

    leadsFaltantesDatos.forEach(r => {
      const cedula = r[1] ? limpiarDoc(r[1]) : '';
      const correo = (r[2] && esCorreoValido(String(r[2]))) ? limpiarCorreo(r[2]) : '';
      const placa = r[4] ? limpiarPlaca(r[4]) : '';

      if (cedula && !mapLF_CC.has(cedula)) mapLF_CC.set(cedula, r);
      if (correo && !mapLF_Correo.has(correo)) mapLF_Correo.set(correo, r);
      if (placa && !mapLF_Placa.has(placa)) mapLF_Placa.set(placa, r);
    });
  }
  ...
  const resultadosFinales = datosEmisiones.map(r => {
    ...
    let countLF_CC = 0, countLF_Placa = 0, countLF_Correo = 0; 
    ...
    if (!foundRowTLC && !foundRowBASES && !foundRowNOV) {
      if (doc && mapLF_CC.has(doc)) {
        foundRowLF = mapLF_CC.get(doc);
      }
      if (placa && !foundRowLF && mapLF_Placa.has(placa)) {
        foundRowLF = mapLF_Placa.get(placa);
      }
      if (correo && !foundRowLF && mapLF_Correo.has(correo)) {
        foundRowLF = mapLF_Correo.get(correo);
      }
    }
    ...
    if (foundRowLF || (doc && mapLF_CC.has(doc)) || (placa && mapLF_Placa.has(placa)) || (correo && mapLF_Correo.has(correo))) {
      if (doc && mapLF_CC.get(doc)) countLF_CC = 1;
      if (placa && mapLF_Placa.get(placa)) countLF_Placa = 1;
      if (correo && mapLF_Correo.get(correo)) countLF_Correo = 1;
    }
    ...
    if (foundRowTLC) {
      ...
      fuenteFinal = foundRowLF[8] || 'FALTANTES';
      medioFinal = foundRowLF[12] || '';
      campañaFinal = foundRowLF[9] || '';
      adnameFinal = ''; 
      fechaFinal = formatearFecha(foundRowLF[11]);
    }


    const totalConteo = countTLC_CC + countTLC_Placa + countTLC_Correo + countBASES_CC + countBASES_Correo + countNOV_CC + countNOV_Placa + countNOV_Correo + countLF_CC + countLF_Placa + countLF_Correo;
    
    const conteos = [
      countTLC_CC, countTLC_Placa, countTLC_Correo, 
      countBASES_CC, countBASES_Correo, 
      countNOV_CC, countNOV_Placa, countNOV_Correo,
      countLF_CC, countLF_Placa, countLF_Correo 
    ];
  ...
  const nuevosEncabezados = [
    ...
    "Faltantes CC", "Faltantes Placa", "Faltantes Correo", 
    "Total Leads", "Ventas",
